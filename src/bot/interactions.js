...
function createServer() {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (!child) {
        ...
        if (applicationId && interactionToken) {
          await sendFollowup(applicationId, interactionToken, 'A tip is already being processed. Please wait.');
        }
      }
    } catch (e) {
  ...
  server.listen(port, () => {
    console.log(`Discord interactions server listening on :${port}/interactions`);
  });
}
